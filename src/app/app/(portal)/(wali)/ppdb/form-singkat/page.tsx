...
import { submitFormSingkat } from '@/features/ppdb/actions/ppdb-actions'
...
const STEPS = [
  ...
  { step: 3, label: 'Form Lengkap & Review' },
]
...
export default function FormSingkatPage() {
  ...
  useEffect(() => {
    async function fetchPeriodes() {
      try {
        ...
        setActivePeriode((current) => current || data[0]?.id || '')
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (typeof periode?.unitId === 'string' && periode.unitId.length > 0) {
      formData.append('unit_id', periode.unitId)
    }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.card}>
        <div className={styles.cardHeader}>
          ...
          <p className={styles.cardSubtitle}>Lengkapi data awal untuk mendapatkan nomor pendaftaran dan lanjut ke invoice formulir.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <button type="submit" disabled={loading || fetchingPeriodes} className={styles.submitBtn}>
            {loading
              ...
              : <><span>Buat Pendaftaran & Lanjut ke Invoice</span><ChevronRight size={20} /></>
            }
          </button>
        </form>
      </div>
