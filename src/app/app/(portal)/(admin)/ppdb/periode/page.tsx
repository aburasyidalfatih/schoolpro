...
export default function PeriodePpdbPage() {
  const [data, setData] = useState<PeriodeRow[]>([])
  ...
  const openEditModal = (row: PeriodeRow) => {
    setEditId(row.id)
    setFormData({
      ...
      biayaPendaftaran: row.pengaturan?.biayaPendaftaran ? String(row.pengaturan.biayaPendaftaran) : '', isActive: row.isActive,
    })
    setErrorMsg(''); setIsModalOpen(true)
  }
