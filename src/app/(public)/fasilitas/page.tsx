...
import { getWebsiteFasilitas } from '@/features/website/lib/website-data'
    ...
    <>
      ...
      <section className="py-12 lg:py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {data.map((item) => {
            const href = item.slug ? `/fasilitas/${item.slug}` : '#'
            return (
              <Link key={item.id} href={href} className="block group">
                <div className="relative h-72 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    ...
                    {item.kapasitas && <p className="text-xs text-white/70 mt-0.5">Kapasitas: {String(item.kapasitas)}</p>}
                  </div>
                </div>
              </Link>
            )
