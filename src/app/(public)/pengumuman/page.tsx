...
import { getWebsitePengumuman } from '@/features/website/lib/website-data'
    ...
    <>
      ...
      <section className="py-12 lg:py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto grid gap-5">
          {data.map((item) => {
            const config = priorityConfig[item.prioritas || 'normal'] || priorityConfig.normal
            const href = item.slug ? `/pengumuman/${item.slug}` : '#'
            const preview = getAnnouncementPreview(item.ringkasan || '', item.konten || '')
            return (
