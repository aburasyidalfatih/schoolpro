...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const item = await prisma.blog.findFirst({ where: { tenantId: tenant.id, slug } })
  if (!item) return notFound()

  return (
    <>
      <PageHeader title={item.judul} breadcrumbs={[{ label: 'Blog Guru', href: '/blog' }, { label: item.judul }]} />
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {item.penulis && (
            <div className="flex items-center gap-4 p-5 rounded-2xl mb-8" style={{ background: 'var(--skin-surface)' }}>
              ...
              <div>
                ...
                {item.bioPenulis && <p className="text-sm" style={{ color: 'var(--skin-text-muted)' }}>{item.bioPenulis}</p>}
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            {item.tags?.split(',').map((tag) => (
              <span key={tag} className="text-xs px-3 py-1.5 rounded-full border" style={{ borderColor: 'var(--skin-border)', color: 'var(--skin-text-muted)' }}>#{tag.trim()}</span>
            ))}
          </div>
