...
export default async function AgendaDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const details = [
    ...
    (item as any).waktu && { icon: <Clock className="h-4 w-4" />, label: 'Waktu', value: (item as any).waktu },
    ...
    (item as any).kontakPerson && { icon: <Phone className="h-4 w-4" />, label: 'Kontak', value: (item as any).kontakPerson },
