    ...
    <>
      ...
      <section className="py-12 lg:py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {data.map((item) => {
            const href = (item as any).slug ? `/ekskul/${(item as any).slug}` : '#'
            const open = (item as any).pendaftaranBuka ?? true
            return (
                  ...
                  <div className="p-5">
                    {(item as any).kategori && (
                      <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full"
                        style={{ background: 'var(--skin-surface)', color: 'var(--skin-primary)' }}>{(item as any).kategori}</span>
                    )}
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs" style={{ color: 'var(--skin-text-muted)' }}>
                      {(item as any).hari && <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {(item as any).hari}</span>}
                      {(item as any).jumlahAnggota !== undefined && (
                        <span className="flex items-center gap-1"><Users className="h-3 w-3" /> {(item as any).jumlahAnggota}/{(item as any).maxAnggota}</span>
                      )}
                    </div>
                  </div>
                </div>
