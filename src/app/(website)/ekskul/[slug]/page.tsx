...
export default async function EkskulDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const item = await prisma.ekskul.findFirst({ where: { tenantId: tenant.id, slug } })
  if (!item) return notFound()

  const jumlah = (item as any).jumlahAnggota || 0
  const max = (item as any).maxAnggota || 30
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {(item as any).kategori && (
              <span className="text-xs font-bold px-3 py-1.5 rounded-full text-white" style={{ background: 'var(--skin-primary)' }}>{(item as any).kategori}</span>
            )}
            {open ? (
            ...
            {(item as any).hari && (
              <div className="flex items-center gap-3 p-4 rounded-xl" style={{ background: 'var(--skin-surface)' }}>
                ...
                <div>
                  ...
                  <p className="font-semibold text-sm" style={{ color: 'var(--skin-text-heading)' }}>{(item as any).hari}</p>
                </div>
              </div>
            )}
            {item.jadwal && (
