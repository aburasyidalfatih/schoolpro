...
import { ArrowRight, Trophy, Medal, ChevronLeft, ChevronRight } from 'lucide-react';
import SectionTitle from '@/components/public/shared/SectionTitle';
import { Badge } from '@/components/public/shared/Badge';
    ...
    <section className="py-16 lg:py-24 overflow-hidden" style={{ background: 'var(--skin-surface)' }}>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <button
          ...
          className="hidden md:flex absolute -left-4 lg:-left-10 xl:-left-14 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-xl border-2 items-center justify-center transition-all hover:shadow-md focus-visible:outline-2 disabled:opacity-30 disabled:cursor-not-allowed backdrop-blur-md"
        ...
        <button
          ...
          className="hidden md:flex absolute -right-4 lg:-right-10 xl:-right-14 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-xl items-center justify-center text-white transition-all hover:shadow-md focus-visible:outline-2 disabled:opacity-30 disabled:cursor-not-allowed"
        ...
        {/* Gradient fade edges */}
        {canScrollLeft && (
          <div className="absolute left-0 top-0 bottom-0 w-12 lg:w-16 z-10 pointer-events-none"
            style={{ background: 'linear-gradient(to right, var(--skin-surface, #f8fafc), transparent)' }} />
        )}
        {canScrollRight && (
          <div className="absolute right-0 top-0 bottom-0 w-12 lg:w-16 z-10 pointer-events-none"
          ...
          {prestasi.map((item, i) => (
            ...
            >
              <Link href={`/prestasi/${item.slug}`} className="block group h-full">
                <div className="h-full rounded-2xl shadow-sm hover:shadow-xl transition-all overflow-hidden border backdrop-blur-md"
      ...
      <div className="flex items-center justify-center gap-4 mt-6">
        <div className="flex items-center justify-center gap-3 md:hidden">
            <button
              onClick={() => scroll('left')}
              disabled={!canScrollLeft}
            className="w-10 h-10 rounded-xl border-2 flex items-center justify-center transition-all focus-visible:outline-2 disabled:opacity-30 disabled:cursor-not-allowed backdrop-blur-md"
            style={{ borderColor: 'var(--skin-primary)', color: 'var(--skin-primary)', background: 'var(--skin-card-bg)' }}
            aria-label="Prestasi sebelumnya"
          >
            <ChevronLeft className="h-5 w-5" />
