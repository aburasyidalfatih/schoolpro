...
1. Jangan overwrite repo lama secara buta
2. Buat baseline commit yang merepresentasikan `schoolpro-dev` saat ini
3. Push ke branch terpisah dulu untuk review
4. Setelah lolos review, merge atau fast-forward sesuai strategi repo
5. Tag commit kandidat release sebelum deploy production

## Strategi Deploy Production Yang Disarankan
1. Freeze perubahan production selama window deploy
2. Sinkronkan source yang sudah disetujui ke `/var/www/schoolpro`
3. Install dependency jika diperlukan
4. Jalankan:
```bash
npm run build
```
5. Jalankan migration hanya jika paket release memang mencakup schema change dan backup DB sudah dibuat
6. Restart proses production:
```bash
pm2 restart schoolpro
```
7. Jalankan smoke test production segera setelah restart
